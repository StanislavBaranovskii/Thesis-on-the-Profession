...
Для создания инфраструктуры использовались следующие продукты:
- Ansible 2.15.1
- Python 3.10.6
...
Структура иописание назначения каталогов проекта:
...
Выбор версий Elasticsearch и Filebeat


### Сеть

..Во время создания каталога проекта в облаке (yc resource-manager folder create) автоматически создается VPC
...
Группа безопасности для web:
- входящий трафик на порт 80 (и 8080) по TCP с любого ip (CIDR 0.0.0.0/0) - web-страница
...
- входящий трафик на порт 22 по TCP(SSH) с внутреннего ip vm-grafana (bastion) (CIDR 10.128.0.22/24)

? Группа безопасности для Prometheus:
- входящий трафик на порт 9090 по TCP с внутреннего ip (CIDR 10.0.0.0/24) - запросы от Grafana
- входящий трафик на порт 22 по TCP(SSH) с внутреннего ip vm-grafana (bastion) (CIDR 10.128.0.22/24)

? Группа безопасности для Grafana + Basion:
- входящий трафик на порт 3000 по TCP с любого ip (CIDR 0.0.0.0/0) - web-страница
- входящий трафик на порт 22 по TCP(SSH) с любого ip (CIDR 0.0.0.0/0) - bastion
- исходящий трафик на порт 22 по TCP(SSH) на внутренний ip (CIDR 10.0.0.0/24) - bastion
